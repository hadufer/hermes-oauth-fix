...
- The system prompt loses its most distinctive Hermes signatures: the `<available_skills>` XML block, the `Host:` / `Shell:` / `Conversation started:` blocks, the `# Claude Code Persona` header, NousResearch URLs, the bare words "Hermes" and "Nous" (both have a `\b…\b` catch-all), the WSL environment hint, `hermes` CLI references, and the "You are X" platform intros. Functional guidance stays put (memory tool, skill loading, MEDIA tags, persona).
...
- Skill *names* that carry a product token (e.g. `debugging-hermes-tui-commands`, or the `hermes-agent` help skill) are disguised in the catalog (`hermes` → `claude`, `nous` → `anthropic`) with the disguise recorded in a reverse map — the same bijection idea as the tool rename. The swap is word-bounded and case-insensitive, so a `Hermes-Toolkit` slug is handled here (instead of being mangled into `Claude Code-Toolkit` by the bare-word catch-all) while a name that merely *contains* the letters — `luminous-ui`, `ominous-mode` — is left alone. On the way back, the `name` argument of `skill_view` / `skill_manage` calls is mapped from the disguised name to the real on-disk name, so loading a disguised skill still works. Skill descriptions and any `.hermes/` paths in them are sanitized too.
...
- Lowercase path/env-var leaks get rewritten: `.hermes/` → `.claude/` (covers both `~/.hermes/` and bare `.hermes/...` paths that show up in skill descriptions), `$HERMES_*` → `$CLAUDE_*`, `.hermes.md` → `.claude.md`, `HERMES.md` → `CLAUDE.md`, `HERMES_HOME` → `CLAUDE_HOME`. Inline `hermes <cmd>` references (setup, status, config, tools, kanban, etc.) get rewritten to `claude <cmd>`.
...
Re-running `python install.py` when already patched is safe: it restores the pristine original from the backup and re-patches with the current rules, so updating the rule set (after a Hermes update) is just a matter of re-running it — no manual `--uninstall` first. The backup is also refreshed from the live file on every (re-)patch, which matters when Hermes auto-updates: the updater overwrites the patched files with a fresh upstream version and leaves the *old* `.oauth-fix.bak` behind. Re-running the installer re-applies the patch on top of the new version and captures the correct new original, so a later `--uninstall` restores the right files instead of silently downgrading to a previous Hermes version.
...
Skill *names* that carry a product token are disguised in the catalog and mapped back on `skill_view` / `skill_manage` arguments (see *What it changes*), so they no longer leak and still load. The one edge case is collision: if disguising a skill (`hermes` → `claude`) would produce a name that already belongs to a *different* real skill, or two skills would disguise to the same name, the patch leaves that one undisguised rather than create an ambiguous reverse mapping. In that (rare) case the token rides along for that single skill. To check, run with `HERMES_OAUTH_FIX_DUMP_DIR` set (see *Diagnosing new leaks*) and grep the dump for `hermes` / `nous`; if a skill name still shows up, rename its directory under `~/.hermes/skills/` to something without the product name (and without colliding with another skill).
