...
- Skill *names* that carry a product token (e.g. `debugging-hermes-tui-commands`, or the `hermes-agent` help skill) are disguised in the catalog (`hermes` → `claude`, `nous` → `anthropic`) with the disguise recorded in a reverse map — the same bijection idea as the tool rename. On the way back, the `name` argument of `skill_view` / `skill_manage` calls is mapped from the disguised name to the real on-disk name, so loading a disguised skill still works. Skill descriptions and any `.hermes/` paths in them are sanitized too.
