...
Your own skill *names* are not rewritten. The skills catalog (the reformatted `<available_skills>` block) lists every installed skill by name, and the model loads a skill by passing that exact name to `skill_view(name=...)`. Rewriting `hermes` → `claude` inside a skill name would make the model request a name that doesn't exist on disk, so the load would silently fail — there's no reverse-mapping for skill-name arguments the way there is for tool names. The consequence: if you have a skill whose slug contains `hermes` or `nous` (e.g. `debugging-hermes-tui-commands`), that token rides along on every request inside the catalog. Skill *descriptions* and any `.hermes/` paths in them are sanitized; only the identifier itself is untouched. If you want it gone, rename the skill on disk (its directory under `~/.hermes/skills/`) to something without the product name. Run with `HERMES_OAUTH_FIX_DUMP_DIR` set (see *Diagnosing new leaks*) and grep the dump for `hermes` to find any offenders.
